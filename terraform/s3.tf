...
resource "aws_s3_bucket_versioning" "bucket_versioning_validated_data" {
  bucket = aws_s3_bucket.validated-data.id
...
resource "aws_s3_bucket_server_side_encryption_configuration" "bucket_crypto_conf_validated" {
  bucket = aws_s3_bucket.validated-data.bucket
  rule {
...
resource "aws_s3_bucket_public_access_block" "validated_data_block" {
  bucket                  = aws_s3_bucket.validated-data.id
...
resource "aws_s3_bucket_public_access_block" "malformed_data_block" {
  bucket                  = aws_s3_bucket.malformed_data.id
  block_public_acls       = true
  block_public_policy     = true
  ignore_public_acls      = true
